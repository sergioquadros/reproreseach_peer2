...
Of course the most frequent events may not be the most fatal and neither that bring higher losses and therefore not serve as a compass for government action.
...
So the annual fatalities spread between `r format(min(by_year$deaths+by_year$hurts),digits=3)` and `r format(max(by_year$deaths+by_year$hurts),digits=3)` citizens and their mean was `r format(mean(by_year$deaths+by_year$hurts),digits=3)` $\pm $ `r format(sd(by_year$deaths+by_year$hurts),digits=3)` citizens.
